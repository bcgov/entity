var dashboardCommands = {
    verifyTombstone: function (coopObject) {
        return this.waitForElementVisible('@entityName')
               ...
               .assert.containsText('@filingHistoryHeader','Recent Filing History (0)')
               .assert.containsText('@noFilingsMessage','You have no filing history');
    },
    ...
    verifyDirectorCount: function (coopObject) {
        return this.assert.containsText('@currentDirectorsHeader','Current Directors')
               .expect.elements('div.director-info').count.to.equal(coopObject.director_count);
    },
